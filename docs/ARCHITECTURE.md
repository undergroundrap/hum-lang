...
Resolution doctrine: checked scope, definition, reference, and place identity comes before execution, type checking, effect checking, ownership, borrowing, editor go-to-definition, debugger facts, and IR emission. `hum ir-readiness` must consume the checked resolver summary and block candidates with resolver errors before any lowering claim. See [HUM_RESOLVE_SCHEMA.md](HUM_RESOLVE_SCHEMA.md) and [decisions/0011-add-checked-resolver-before-execution.md](decisions/0011-add-checked-resolver-before-execution.md).
