...
| Section hover | Adapter-ready | `hum syntax`, `hum.syntax_surface.v0`, `section_catalog` | Explain section keywords such as `why:`, `does:`, `needs:`, `ensures:`, `cost:`, and `protects:`. | This should be identical across editors. |
...
| Semantic-token legend | Current | `hum syntax`, `semantic_tokens` | Use the shared legend for client themes and adapter mapping. | Actual semantic-token ranges are deferred until source ranges are precise. |
