...
- returned-view dependencies of the shape `ResultType from parameter` when the source is a task parameter and each V0 return visibly returns that bare parameter
- rejected returned-view dependencies as `H0805` when the source is a local, an internal reference such as `parser.buffer`, an unknown name, or a complex/nonmatching returned expression
...
- It may report recognized V0 ownership facts, narrow Transaction-shaped linear-resource path facts, returned-view dependencies from parameters, and explicit blockers only.
