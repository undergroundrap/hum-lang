...
| `needs:` | current | preconditions, predicate v1 runtime entry checks, and generated test obligations |
| `ensures:` | current | postconditions, predicate v1 runtime exit checks with `old(...)` pre-state capture, and generated test obligations |
...
Current executable blame semantics are deliberately small and explicit: parseable predicate `needs:` lines run at task entry and blame the caller when false; parseable predicate `ensures:` lines run after successful return and blame the task when false. Predicate v1 is one canonical operator comparison such as `b != 0`, `result == a + b`, or `result.done == true`, with task parameters and direct field paths rooted in those parameters available in both sections and `result` plus direct field paths rooted in `result` available only in `ensures:`. Predicate v1 adds two operand forms grown from the recorded contract wishlist: `old(place)` in `ensures:` only, capturing a parameter or parameter field value at task entry so pre-state contracts such as `result.x == old(point.y)` and preservation contracts such as `result.title == old(item.title)` are checkable, and `list_len(place)` in both sections for list-length comparisons such as `list_len(result) == 3`. `old(...)` outside `ensures:`, `old(result)`, quantifiers, boolean connectives, list-content claims, and text literals remain outside the vocabulary. Prose lines remain visible intent and produce an unchecked-contract warning under `hum run`, not an error.
