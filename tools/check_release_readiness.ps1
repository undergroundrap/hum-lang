...
foreach ($required in @('hum.doctor.v0', 'hum doctor --format json', 'current_directory', 'text_hygiene_policy', 'public_readiness_policy', 'preflight_script', 'clean_checkout_smoke', 'tag_readiness', 'guarded hosted preflight', 'tools/check_clean_checkout.ps1', 'tools/check_tag_readiness.ps1')) {
...
foreach ($required in @('workflow_dispatch:', 'branches:', '- main', 'tags:', "- 'v*'", 'concurrency:', 'cancel-in-progress: true', 'timeout-minutes: 15', 'actions/checkout@v7', 'actions/cache@v6', 'CARGO_HOME:', 'Cache Cargo artifacts', '.cargo-home/registry', '.cargo-home/git', 'target', 'restore-keys:', 'tools/check_all.ps1', 'windows-latest', 'ubuntu-latest')) {
