        ...
        match kind {
            ...
            "empty_not_checked" => {
                let _ = facts
                    .checked_empty_sets
                    .get_mut(index)
                    .map(|value| *value = "state_not_checked");
            }
            "empty_unsupported" => {
                let _ = facts
                    .checked_empty_sets
                    .get_mut(index)
                    .map(|value| *value = "state_unsupported_and_blocked");
            }
            "empty_substituted" | "empty_corrupted" => {
                let _ = facts
                    .checked_empty_sets
                    .get_mut(index)
                    .map(|value| *value = "foreign_checked_empty_state");
            }
            "empty_duplicate" => {
...
"#;
    ...
    fn minimal_add_backend_facts_are_complete_but_ir_verify_blocked() {
        fn subject(source: &str) -> (Program, Vec<crate::diagnostic::Diagnostic>) {
            subject_at("examples/core/minimal_add.hum", source)
        }

        fn subject_at(path: &str, source: &str) -> (Program, Vec<crate::diagnostic::Diagnostic>) {
        ...
        fn assert_public_identity_rewrite(
            before: &[(&str, String); 10],
            after: &[(&str, String); 10],
            rewritten_path: &str,
        ) {
            for ((before_name, before), (after_name, after)) in before.iter().zip(after) {
                assert_eq!(before_name, after_name);
                assert_ne!(
                    before, after,
                    "complete public report did not change: {before_name}"
                );
            }
            let before = before
                .iter()
            ...
            for (field, old, new) in [
                ...
                ("item name", "\"name\": \"add\"", "\"name\": \"combine\""),
                (
                    "full-type item ID",
                    "hum_full_type_item_task_add_3",
                    "hum_full_type_item_task_combine_3",
                ),
                (
                    "effect item ID",
                    "hum_effect_item_task_add_3",
                    "hum_effect_item_task_combine_3",
                ),
                (
                    "ownership item ID",
                    "hum_ownership_item_task_add_3",
                    "hum_ownership_item_task_combine_3",
                ),
                (
                    "resource item ID",
                    "hum_resource_item_add_3",
                    "hum_resource_item_combine_3",
                ),
                (
                    "resource check ID",
                    "hum_resource_check_add_allocation_free_visibility_3",
                    "hum_resource_check_combine_allocation_free_visibility_3",
                ),
                (
                    "profile item ID",
                    "hum_profile_item_task_add_3",
                    "hum_profile_item_task_combine_3",
                ),
                (
                    "profile check ID",
                    "hum_profile_check_task_add_normal_profile_policy_3",
                    "hum_profile_check_task_combine_normal_profile_policy_3",
                ),
            ] {
                assert!(before.contains(old), "canonical public {field} missing");
                assert!(
                    !before.contains(new),
                    "rewritten public {field} already canonical"
                );
                assert!(
                    !after.contains(old),
                    "canonical public {field} survived rewrite"
                );
                assert!(after.contains(new), "rewritten public {field} missing");
            }
        }
        ...
        fn access_with_foreign_lineage(
            report_program: &Program,
            report_diagnostics: &[crate::diagnostic::Diagnostic],
            query_program: &Program,
        ) -> Option<Vec<String>> {
            let item = &query_program.files[0].items[0];
            let crate::ast::Item::Task(task) = item else {
                return None;
            };
            crate::profile_check::with_profile_for_ir_readiness(
                ...
                |profile_access| {
                    let profile =
                        profile_access.canonical_minimal_add_for(item, &task.body_syntax[0])?;
                    super::backend_facts::issue(
                        query_program,
                        item,
                        &task.body_syntax[0],
                        profile,
        ...
        let canonical_public_reports = public_reports(&program, &diagnostics);

        let absent_source = "module tests.wo18.rewritten\n\ntask combine(left: UInt, right: UInt) -> UInt {\n  allocates:\n    nothing\n\n  does:\n    return left + right\n}\n";
        let (absent_program, absent_diagnostics) =
            subject_at("tests/wo18/absent_public_lineage.hum", absent_source);
        assert_public_chain_unblocked(&absent_program, &absent_diagnostics);
        let absent_public_reports = public_reports(&absent_program, &absent_diagnostics);
        assert_public_identity_rewrite(
            &canonical_public_reports,
            &absent_public_reports,
            "tests/wo18/absent_public_lineage.hum",
        );
        let absent_json = absent_public_reports
            .iter()
            .filter(|(name, _)| name.ends_with(".json"))
            .map(|(_, report)| report.as_str())
            .collect::<String>();
        for (field, old, new) in [
            ("expression/target text", "a + b", "left + right"),
            (
                "expected type",
                "\"expected_type\": \"Int\"",
                "\"expected_type\": \"UInt\"",
            ...
            (
                "type source",
                "\"type_source\": \"verified_canonical_minimal_add_type_v0\"",
                "\"type_source\": \"additive_expression_v0\"",
            ),
        ] {
            assert!(
                canonical_public_reports[1].1.contains(old),
                "canonical public {field} missing"
            );
            assert!(
                !absent_json.contains(old),
                "canonical public {field} survived rewrite"
            );
            assert!(
                absent_json.contains(new),
                "rewritten public {field} missing"
            );
        }
        assert!(
            access(&absent_program, &absent_diagnostics).is_none(),
            "coherent all-public-field rewrite must not replace absent private lineage"
        );

        let foreign_source = "module tests.wo18.rewritten\n\ntask combine(a: Int, b: Int) -> Int {\n  allocates:\n    nothing\n\n  does:\n    return a + b\n}\n";
        let (foreign_report_program, foreign_report_diagnostics) =
            subject_at("tests/wo18/foreign_public_lineage.hum", foreign_source);
        let (foreign_query_program, foreign_query_diagnostics) =
            subject_at("tests/wo18/foreign_public_lineage.hum", foreign_source);
        assert_public_chain_unblocked(&foreign_report_program, &foreign_report_diagnostics);
        assert_public_chain_unblocked(&foreign_query_program, &foreign_query_diagnostics);
        let foreign_report_fields =
            public_reports(&foreign_report_program, &foreign_report_diagnostics);
        let foreign_query_fields =
            public_reports(&foreign_query_program, &foreign_query_diagnostics);
        assert_public_identity_rewrite(
            &canonical_public_reports,
            &foreign_report_fields,
            "tests/wo18/foreign_public_lineage.hum",
        );
        assert!(
            foreign_report_fields.iter().all(
                |(_, report)| report.contains("combine") && !report.contains("minimal_add.hum")
            )
        );
        assert_eq!(foreign_report_fields, foreign_query_fields);
        assert!(access(&foreign_report_program, &foreign_report_diagnostics).is_some());
        assert!(access(&foreign_query_program, &foreign_query_diagnostics).is_some());
        assert!(
            access_with_foreign_lineage(
                &foreign_report_program,
                &foreign_report_diagnostics,
                &foreign_query_program,
            )
            .is_none(),
            "byte-identical complete public fields must not replace foreign private lineage"
        );
