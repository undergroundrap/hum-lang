...
const IR_LAYERS: &[IrLayer] = &[
    ...
    IrLayer {
        id: "hum_ir",
        stage: 4,
        status: "verified-input-ready",
        role: "verified target-independent backend input behind a sealed compiler capability",
    },
    IrLayer {
...
const NON_GOALS_V0: &[&str] = &[
    "no durable verified-input authority",
...
mod tests {
    ...
    fn text_ir_contract_lists_semantic_layers() {
        ...
        assert!(text.contains("4. hum_ir [verified-input-ready]"));
        assert!(text.contains(
            "verified target-independent backend input behind a sealed compiler capability"
        ));
    ...
    fn json_ir_contract_is_machine_readable() {
        ...
        assert!(json.contains("\"status\": \"verified-input-ready\""));
        assert!(json.contains("\"no durable verified-input authority\""));
    }
}
