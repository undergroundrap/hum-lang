...
#[cfg(test)]
thread_local! {
    static WO19_STAGE_CORRUPTION: std::cell::Cell<Option<(&'static str, &'static str)>> = const { std::cell::Cell::new(None) };
}

#[cfg(test)]
pub(crate) fn set_wo19_stage_corruption(stage: &'static str, kind: &'static str) {
    WO19_STAGE_CORRUPTION.with(|active| assert_eq!(active.replace(Some((stage, kind))), None));
}

#[cfg(test)]
pub(crate) fn take_wo19_stage_corruption(stage: &str) -> Option<&'static str> {
    WO19_STAGE_CORRUPTION.with(|active| {
        active
            .get()
