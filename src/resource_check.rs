...
pub(crate) struct VerifiedMinimalAddResource<'report> {
    pub(crate) ownership: ownership_check::VerifiedMinimalAddOwnership<'report>,
...
#[cfg(test)]
fn corrupt_wo18_report_for_test(report: &mut ResourceCheckReport) {
    let Some(kind) = crate::type_check::take_wo18_stage_corruption("resource_check") else {
        return;
    };
    match kind {
        "missing" => {
