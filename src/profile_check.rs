...
#[cfg(test)]
fn corrupt_wo19_report_for_test(report: &mut ProfileCheckReport) {
    let Some(kind) = crate::type_check::take_wo19_stage_corruption("profile_check") else {
        return;
    };
    match kind {
        "missing" => {
...
impl<'report> ProfileIrReadinessReportAccess<'report> {
    pub(crate) fn canonical_minimal_add_for(
        ...
        (declaration.source_section == "default"
            ...
            .then_some(VerifiedMinimalAddProfile(VerifiedMinimalAddProfileProof {
                resource,
                declaration,
            }))
    }
