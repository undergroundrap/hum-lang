...
pub(crate) struct VerifiedMinimalAddFullType<'report>(
    pub(crate) core_verify::VerifiedCanonicalMinimalAddTypeResult<'report>,
);
...
#[cfg(test)]
fn corrupt_wo18_report_for_test(report: &mut FullTypeCheckReport) {
    let Some(kind) = type_check::take_wo18_stage_corruption("full_type_check") else {
        return;
    };
    match kind {
        "missing" => {
...
"#,
    ...
    fn minimal_add_backend_fact_handoff_is_exact_and_borrowed() {
        ...
        let delivered = super::with_full_type_for_effect(&program, &diagnostics, |access| {
            ...
            let identity = result.0.backend_identity();
            assert_eq!(
            ...
            assert_eq!(result.0.core_prerequisite_names().count(), 7);
