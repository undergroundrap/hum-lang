...
pub fn digest(input: &[u8]) -> Option<[u8; 32]> {
    digest_checked(input).ok()
}

pub fn digest_checked(input: &[u8]) -> Result<[u8; 32], InputTooLarge> {
    let byte_length = u64::try_from(input.len()).map_err(|_| InputTooLarge)?;
    let bit_length = checked_bit_length(byte_length)?;
    let zero_padding =
        usize::try_from((64 - ((byte_length + 9) % 64)) % 64).map_err(|_| InputTooLarge)?;
    let padded_length = input
        .len()
        .checked_add(9)
        .and_then(|v| v.checked_add(zero_padding))
        .ok_or(InputTooLarge)?;
...
pub fn checked_bit_length(byte_length: u64) -> Result<u64, InputTooLarge> {
...
pub fn lowercase_hex(digest: &[u8; 32]) -> String {
...
pub fn digest_hex(input: &[u8]) -> String {
    lowercase_hex(&digest(input).expect("input fits the SHA-256 length domain"))
}
...
mod tests {
    ...
    #[test]
    fn shared_sha256_known_answer_and_boundary_matrix_is_exact() {
        let vectors = [
            (
                Vec::new(),
