use crate::{
    command::EvidenceProfile,
    identity::CandidateIdentity,
    shell::{ExecutableBinding, ShellEnvironment},
};
...
pub(crate) struct PwshIdentity {
    pub(crate) version: String,
    pub(crate) sha256: String,
}

pub(crate) fn authenticate_pwsh7(
    executable: &ExecutableBinding,
    environment: &ShellEnvironment,
) -> Result<PwshIdentity, String> {
    let output = executable
        ...
        .arg("--version")
        .output()
        .map_err(|e| format!("pwsh_version_launch: {e}"))?;
    let stdout =
        String::from_utf8(output.stdout).map_err(|_| "pwsh_version: stdout is not UTF-8")?;
    let version = stdout
        .trim_end_matches(['\r', '\n'])
        .strip_prefix("PowerShell ")
        .ok_or("pwsh_version: runtime is not authenticated PowerShell 7")?;
    let valid = version.split('.').next() == Some("7")
        && version
            .split('.')
            .all(|part| !part.is_empty() && part.bytes().all(|b| b.is_ascii_digit()));
    if !output.status.success() || !output.stderr.is_empty() || !valid {
        return Err("pwsh_version: runtime is not authenticated PowerShell 7".into());
    }
    executable.reauthenticate()?;
    Ok(PwshIdentity {
        version: version.into(),
        sha256: executable.sha256().into(),
    })
}
...
impl JobSummary {
    ...
    pub fn binding(&self) -> String {
        let mut values=vec![(if self.schema==SCHEMA{JOB_POLICY}else{JOB_POLICY_V2}).to_string(),self.generator.clone(),self.commit.clone(),self.parent.clone(),self.tree.clone(),self.candidate_manifest.clone(),self.raw_additions.to_string(),self.raw_deletions.to_string(),self.whitespace_additions.to_string(),self.whitespace_deletions.to_string(),self.cargo_lock_sha256.clone(),self.dependency_closure_sha256.clone(),self.configuration_sha256.clone(),self.platform.clone(),self.target.clone(),self.toolchain.clone(),self.compiler_sha256.clone(),self.producer_executable_sha256.clone()];
        if self.schema==JOB_SCHEMA_V2{values.extend([self.orchestration_runtime.clone(),self.orchestration_version.clone(),self.orchestration_executable_sha256.clone()]);}
        values.extend([self.profile.clone(),self.workflow.clone(),self.event.clone(),self.run_id.to_string(),self.run_attempt.to_string(),self.job_id.to_string(),self.checkout_sha.clone(),self.classifier_mode.clone(),self.classifier_reason.clone(),self.anchor.clone(),self.transitions.clone(),self.selector_ledger_sha256.clone(),self.selector_count.to_string(),self.mutation_ledger_sha256.clone(),self.mutation_count.to_string(),self.suite_count.to_string(),self.readiness.clone(),self.hygiene_file_count.to_string(),self.claims.clone(),self.nonclaims.clone(),self.release_version.clone(),self.terminal.clone(),self.exit.to_string(),self.started_ticks.to_string(),self.completed_ticks.to_string(),self.timer_frequency.to_string(),self.duration_ms.to_string(),self.stdout_sha256.clone(),self.stderr_sha256.clone(),self.event_sha256.clone(),self.cleanup.clone()]);
        invocation_hash(&values.iter().map(String::as_str).collect::<Vec<_>>())
    }
    pub fn artifact_name(&self) -> String { format!("hum-evidence-summary-{}-{}-{}-{}-{}", if self.schema==SCHEMA{"v1"}else{"v2"}, self.run_id, self.run_attempt, self.job_id, self.platform) }
