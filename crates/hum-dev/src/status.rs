...
pub struct StatusRequest {
    ...
    pub workorder_repo_path: String,
}
...
pub fn request_from_environment() -> Result<StatusRequest, String> {
    ...
    Ok(StatusRequest {
        ...
        workorder_repo_path: value("HUM_STATUS_WORKORDER_REPO_PATH")?,
    })
}
...
fn run_with_transport<T: Transport>(
    ...
    let result = (|| {
        ...
        let endpoint = format!(
            "repos/{}/contents/{}?ref={}",
            request.repository, request.workorder_repo_path, request.anchor
        );
...
mod tests {
    ...
    fn status_consumes_schema_owned_counts() {
        ...
        for both in [false, true] {
            assert_eq!(
                controlled_case(Corruption::Legacy(both)).unwrap_err(),
                "ubuntu summary artifact-name mismatch"
            );
        }
        assert!(controlled_case(Corruption::None).is_ok());
    }
