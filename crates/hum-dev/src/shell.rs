...
#[cfg(test)]
mod ci_boundary_required_tests {
    use super::ShellEnvironment;
    use std::path::PathBuf;

    #[test]
    fn ci_boundary_required_survives_sanitized_environment() {
        // Decision 0025 amendment (2026-09-23, boolean transport): the fixed
        // profiles gate the status-boundary classifier tests on a boolean
        // computed by the plan/push classify step from the ACCEPTED BASE
        // policy and transported through the evidence launcher's sanitized
        // environment. from_process admits exactly the keys in
        ...
        environment
            .0
            .insert("HUM_CI_BOUNDARY_REQUIRED".into(), "false".into());
