...
| `crates/hum-dev/src/shell.rs` | 316 | 0 | process launch, environment, and stream ownership |
| `crates/hum-dev/tests/cli.rs` | 160 | 20 | Windows/Linux shell corruption tests |
| `tools/check_all.ps1` | 80 | 160 | delegate migrated orchestration |
...
| **Unit C total** | **1,236** | **1,120** | **13 non-borrowable paths** |

| Unit C category | Paths | Max + | Max - |
| --- | ---: | ---: | ---: |
| `hum-dev` Rust and tests | 5 | 676 | 80 |
| PowerShell wrappers/tests | 5 | 380 | 900 |
| Workflow | 1 | 60 | 80 |
| Documentation | 2 | 120 | 60 |
| **Unit C category total** | **13** | **1,236** | **1,120** |

The `shell.rs` +36 exception is solely for the proven readable implementation
of fixed Windows system-command identity: native-system `cmd.exe` is selected
from the OS-derived system directory and may legitimately be Windows-hard-linked
into WinSxS. Substitutable `pwsh`, `link.exe`, `vswhere`, Git, Cargo, and other
executables retain their stricter substitution and link rules. This increase
grants no unrelated feature, test, documentation, workflow, or later-unit
budget, and unused lines cannot be borrowed elsewhere.
...
telemetry `+10,781/-7,738`. This sum is not a cross-unit borrowing pool. Every
